...
export default class Project extends Component {
  ...
  RejectItem( { item, index } ){
    ...
    // Flip the 'clicled' property of the targetPost
    targetPost.reject = !targetPost.reject;
        ...
        <FlatList
          ...
          renderItem={({item, index}) => 
            ...
            <View  onPressItem={this._onPressItem}  >
              <View style={{flex:1, backgroundColor: item.reject ? "#484848" : "#fff" }}>
                <TouchableOpacity onPress={item.reject? console.log("item reject") : this.GetItem.bind(this, item)} >
                  <Image source = {{ uri: item.image }} 
                    style={{
                      height: screen.height / 5,
                      opacity: item.reject ? 0.3:1,
                      margin: 7,
                      borderRadius : 7,
                ...
                <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'center', width:'100%', }}>
                  <Icon name={item.clicked ? "ios-download" :"ios-download-outline"} style={styles.iconStyle}    onPress={()=>item.reject ? console.log("error") :this.SaveItem( { item, index } )} />
                    <Text  style={styles.textView} onPress={item.reject? console.log("item reject") :this.GetItem.bind(this, item)}>{item.title}</Text>
                  <Icon name={item.reject ? "ios-checkmark" :"ios-close"}  style={{color: 'black', width :'10%', paddingLeft: '3%', alignItems: 'center', justifyContent: 'center',color: item.reject ? "green" :"red"}}   onPress={()=>this.RejectItem( { item, index } )} />
                </View>
              </View>
            </View>   
          }
